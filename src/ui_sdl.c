    ...
    *out_pause = 0;
    ...
    while (SDL_PollEvent(&e))
    {
        ...
        if (e.type == SDL_KEYDOWN)
        {
            ...
            int action = settings_find_action(settings, key);
            if (action >= 0)
            {
                *out_has_dir = 1;
                switch (action)
...
static void ui_sdl_draw_world(UiSdl *ui, const Game *g)
{
    int ox, oy;
    compute_layout(ui, &g->board, &ox, &oy);

    // Render board, food, and snake using render_utils
    render_board(ui->ren, &g->board, ui->cell, ox, oy);
    render_food(ui->ren, &g->board.food, 1, ui->cell, ox, oy);
    render_snake(ui->ren, &g->snake, COLOR_SNAKE_HEAD_R, COLOR_SNAKE_HEAD_G, COLOR_SNAKE_HEAD_B,
                 ui->cell, ox, oy, &g->board);
}
static void ui_sdl_draw_hud(UiSdl *ui, const Game *g, const char *player_name, int debug_mode, unsigned int current_tick_ms)
{
    int ox, oy;
    compute_layout(ui, &g->board, &ox, &oy);

    // board rect for overlay center calculations
    ...
    if (ui->text_ok)
    {
        ...
        if (g->combo_count > 0)
        {
            ...
            int prev_count = ui->last_combo_count;
            int prev_tier = ui->last_combo_tier;

            int combo_increased = (g->combo_count > prev_count);
            int tier_up = (tier > prev_tier);

            if (combo_increased)
            {
                // ui->combo_flash_until_ms = now + (tier_up ? 200 : 120);

                // small punch always, stronger on tier-up
                if (tier_up)
                {
                    speedfx_combo_punch(&ui->speedfx, (float)tier, 0.2f);
                }

                float amp = tier_up ? 1.0f : 0.6f;
                speedfx_combo_ring(&ui->speedfx, hx, hy, tier, amp);
            }
...
void ui_sdl_render(UiSdl *ui, const Game *g, const char *player_name, int debug_mode, unsigned int current_tick_ms)
{
    ...
    unsigned int now = (unsigned int)SDL_GetTicks();
    float dt = 1.0f / 60.0f;
    if (ui->fx_last_frame_ms != 0)
    {
        dt = (float)(now - ui->fx_last_frame_ms) / 1000.0f;
        if (dt < 0.0f)
            dt = 0.0f;
        if (dt > 0.1f)
            dt = 0.1f;
    ...
    speedfx_set_viewport(&ui->speedfx, ui->w, ui->h);

    // Compute snake head origin in screen coords (for particle spawn)
    float origin_x = 0.0f, origin_y = 0.0f;
    {
        ...
        {
            Vec2 head = g->snake.segments[0];
            origin_x = (float)(ox + (1 + head.x) * ui->cell + ui->cell * 0.5f);
            origin_y = (float)(oy + (1 + head.y) * ui->cell + ui->cell * 0.5f);
        }
        else
        {
            origin_x = (float)(ui->w / 2);
            origin_y = (float)(ui->h / 2);
        }
    }

    // Compute board rect in screen coords (matches your board_bg)
    SDL_Rect board_rect;
    {
        int ox, oy;
        compute_layout(ui, &g->board, &ox, &oy);
    ...
    SDL_Rect head_cell = {0, 0, 0, 0};
    {
        ...
        {
            Vec2 head = g->snake.segments[0];
            head_cell.x = ox + (1 + head.x) * ui->cell;
            head_cell.y = oy + (1 + head.y) * ui->cell;
            head_cell.w = ui->cell;
            head_cell.h = ui->cell;
        }
    }
    ...
    // Render world to texture if supported; otherwise render directly
    if (ui->world_target)
    {
        ...
        // Copy world with shake offset (translation only)
    ...
    speedfx_render_rings(ui->ren, &ui->speedfx,
                         COLOR_SNAKE_HEAD_R, COLOR_SNAKE_HEAD_G, COLOR_SNAKE_HEAD_B);

    // HUD should be stable (not shaken)
    ui_sdl_draw_hud(ui, g, player_name, debug_mode, current_tick_ms);
...
void ui_sdl_render_menu(UiSdl *ui, const Settings *settings, int selected_index)
{
    ...
    if (ui->text_ok)
    {
        ...
        const char *up = settings_key_name(settings_get_key(settings, SETTING_ACTION_UP));
        const char *down = settings_key_name(settings_get_key(settings, SETTING_ACTION_DOWN));
...
void ui_sdl_render_pause_menu(UiSdl *ui, const Game *g, const char *player_name, int selected_index, int debug_mode, unsigned int current_tick_ms)
{
    ...
    ui_sdl_render(ui, g, player_name, debug_mode, current_tick_ms);
...
void ui_sdl_render_options_menu(UiSdl *ui, const Settings *settings, int selected_index)
{
    ...
    menu_render_standard(ui->ren, &ui->text, "OPTIONS", items, 3, selected_index, ui->w, ui->h);
...
void ui_sdl_render_keybind_prompt(UiSdl *ui, const Settings *settings, SettingAction action)
{
    ...
    // Title
    ui_draw_text_centered(ui->ren, &ui->text, cx, 40, "KEYBINDS");
    ...
    // Current binding
    SDL_Keycode current_binding = settings_get_key(settings, action);
    ...
    if (action > 0)
    {
        ...
        {
            SDL_Keycode bound = settings_get_key(settings, (SettingAction)i);
            char entry[64];
...
void ui_sdl_render_sound_settings(UiSdl *ui, const Settings *settings, const AudioSdl *audio, int selected_index)
{
    ...
    if (ui->text_ok)
    {
        ...
        const char *up = settings_key_name(settings_get_key(settings, SETTING_ACTION_UP));
        const char *down = settings_key_name(settings_get_key(settings, SETTING_ACTION_DOWN));
        const char *left = settings_key_name(settings_get_key(settings, SETTING_ACTION_LEFT));
        const char *right = settings_key_name(settings_get_key(settings, SETTING_ACTION_RIGHT));
...
void ui_sdl_render_multiplayer_online_menu(UiSdl *ui, int selected_index)
{
    ...
    menu_render_standard(ui->ren, &ui->text, "Online Multiplayer", options, 3, selected_index, ui->w, ui->h);
}
...
void ui_sdl_render_host_setup(UiSdl *ui, int selected_index)
{
    ...
    menu_render_standard(ui->ren, &ui->text, "Host Game", options, 2, selected_index, ui->w, ui->h);

    // Draw prompt above the menu items
    int cx = ui->w / 2;
    int prompt_y = ui->h / 2 - 80;
    ui_draw_text_centered(ui->ren, &ui->text, cx, prompt_y, "Private Game?");
...
void ui_sdl_render_join_select(UiSdl *ui, int selected_index)
{
    ...
    menu_render_standard(ui->ren, &ui->text, "Join Game", options, 2, selected_index, ui->w, ui->h);

    // Draw prompt above the menu items
    int cx = ui->w / 2;
    int prompt_y = ui->h / 2 - 80;
    ui_draw_text_centered(ui->ren, &ui->text, cx, prompt_y, "Join Type:");
...
void ui_sdl_render_error(UiSdl *ui, const char *message)
{
    ...
    // Build error message with title
    char full_message[512];
    snprintf(full_message, sizeof(full_message), "Error\n\n%s", message);

    // Use custom background color for error
    SDL_SetRenderDrawColor(ui->ren, 40, 0, 0, 255);
    SDL_RenderClear(ui->ren);

    // Render using menu_render_message for centered text
    int center_x = ui->w / 2;
    int center_y = ui->h / 2;

    ui_draw_text_centered(ui->ren, &ui->text, center_x, ui->h / 3, "Error");
    ui_draw_text_centered(ui->ren, &ui->text, center_x, center_y, message);
...
UiMenuAction ui_sdl_poll_online_lobby(UiSdl *ui, const Settings *settings, int *out_quit)
{
    ...
    while (SDL_PollEvent(&e))
    {
        ...
        if (e.type == SDL_KEYDOWN)
        {
            ...
            // Check for USE key
            if (e.key.keysym.sym == settings->keybindings[SETTING_ACTION_USE])
